#!/usr/bin/env node
// Empties and deletes the "scripts" storage bucket (the one holding original
// uploaded PDFs). Supabase rejects direct SQL `delete from storage.objects` /
...
// path, run once before applying the migration that drops `scripts.pdf_path`.
//
// Usage:
//   node scripts/purge-script-pdfs.mjs              dry run, prints what would be deleted
//   node scripts/purge-script-pdfs.mjs --confirm     actually deletes objects + the bucket
//
...
// Bucket contents are one folder per user ("<user_id>/<script_id>.pdf") —
...
async function main() {
  ...
  console.log('Now apply the migration that drops "scripts.pdf_path".');
}
